...
public:
    ...
    vector<unities::JobsForSprint> getJobsForSprint(int sprintID){
        ...
        std::string s=std::string(std::string("SELECT Person.PersonID, Person.Name, CompleteCount, CompleteEstTime,"
                    ...
                    AND '2017-07-08' WHERE Job.SprintID=")+std::to_string(sprintID)+(" AND Job.PersonID=Person.PersonID GROUP BY Person.PersonID;"));
        std::cout<<s<<std::endl;
    ...
    vector<unities::SprintBase> getSprintsForProject(int id) {
        mysql_query(connector, std::string(std::string("SELECT DISTINCT  Sprint.SprintID, Name FROM Sprint,Job where \
                                           Sprint.SprintID=Job.SprintID AND Sprint.ProjectID=")+std::to_string(id)+(" ORDER BY Sprint.SprintBegin DESC;")).c_str());
        vector<unities::SprintBase> vector;
        if (res = mysql_store_result(connector)) {
            //vector.resize(mysql_num_rows(res));
            while(row = mysql_fetch_row(res)){
